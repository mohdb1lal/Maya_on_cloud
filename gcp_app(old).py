...
# In app.py, replace the entire EnhancedVoiceActivityDetector class with this:
...
class VoiceActivityDetector:
    ...
    def process_chunk(self, audio_chunk: bytes) -> tuple[bool, bool]:
        """
        Processes a single audio chunk and returns speech state.
        ...
        try:
            ...
            is_speech_chunk = rms > self.threshold
            ...
            logger.error(f"VAD process_chunk error: {e}")
            return False, False
...
class FreeSwitchWebSocketHandler:
    ...
    # In app.py -> FreeSwitchWebSocketHandler, replace the existing _handle_audio_stream method

    async def _handle_audio_stream(self, audio_data: bytes):
        if not self.call_active or not audio_data:
            return

        if self.assistant_speaking:
            return

        await self.audio_buffer_manager.add_audio(audio_data)
        ...
        while True:
            ...
            is_currently_speech, speech_just_ended = self.vad.process_chunk(chunk)

            if is_currently_speech:
                if not self.user_speaking:
                    self.user_speaking = True
                    logger.info(f"Call {self.call_id}: USER STARTED SPEAKING")
